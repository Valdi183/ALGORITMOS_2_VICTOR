...
class Song:
    ...
    def __init__(self, id, name, artist, duration, release_date, genres=None):
        ...
        # Verificar el tipo y valor de cada parámetro y lanzar excepción si no se cumplen los requisitos
        ...
        if not isinstance(self.artist, str):
            raise ValueError("Artista debe ser una cadena de caracteres.")
        if not isinstance(self.duration, int) or self.duration <= 10:
            raise ValueError("Duración debe ser un entero mayor que 10.")
        if not isinstance(self.release_date, date) or self.release_date > date.today():
            raise ValueError("Fecha de lanzamiento debe ser una fecha válida.")
